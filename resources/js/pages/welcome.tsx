...
export default function Welcome({ canRegister, stats, catalog }: Props) {
    ...
    useEffect(() => {
        if (isLoggedIn && activeTab === -1) {
            ...
            getFavorites()
                .then(setFavorites)
                .catch(() =>
                    setFavError('Não foi possível carregar os favoritos.'),
                )
    ...
    const activeSubs =
        catalog[activeTab >= 0 ? activeTab : 0]?.subspecialties ?? [];
    const activeSub =
        activeTab >= 0 ? activeSubs[activeSubspecialty] : undefined;
    ...
    async function loadSubCodes(
        ...
        try {
            const [cm, pcs] = await Promise.all([
                counts.cm_count > 0
                    ? getIcd10Cm({ subspecialty_id: subId, page: 1 }).then(
                          (r) => r.data,
                      )
                    : Promise.resolve([] as Icd10Cm[]),
                counts.pcs_count > 0
                    ? getIcd10Pcs({ subspecialty_id: subId, page: 1 }).then(
                          (r) => r.data,
                      )
                    ...
                    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
                        ...
                        <div className="mx-auto max-w-5xl text-center">
                            <span className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-400">
                                ...
                                Classificação Internacional de Doenças — 10.ª
                                Revisão
                            </span>
                            ...
                            <p className="mt-4 text-base text-slate-500 dark:text-slate-400">
                                Pesquise e consulte códigos ICD-10-CM e
                                ICD-10-PCS organizados por especialidade
                                clínica.
                            </p>
                        ...
                        >
                            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg dark:border-slate-700 dark:bg-slate-900">
                                {/* TABS */}
                                <div className="flex border-b border-slate-100 px-4 pt-3 dark:border-slate-800">
                                    <button
                                        type="button"
                                        onClick={() => setSearchType('cm')}
                                        className={`rounded-t-md px-5 py-2 text-sm font-semibold transition ${
                                            searchType === 'cm'
                                ...
                                <div className="flex items-center gap-3 p-4">
                                    ...
                                    <button
                                        ...
                                        className="flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-60"
                                    >
                                        {searchLoading ? (
                                                ...
                                                {searchResults.map((item) => (
                                                    ...
                                                    >
                                                        <button
                                                            ...
                                                            className="flex w-full items-center gap-4 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/60"
                                                        >
                                                            <span className="rounded-md bg-blue-50 px-2 py-1 font-mono text-xs font-bold text-blue-700 dark:bg-blue-950/40 dark:text-blue-400">
                                                                {item.code}
                                                            </span>
                    ...
                    <div className="mb-4 flex items-center justify-between">
                        <div>
                            ...
                            <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                                Apenas especialidades com códigos ICD-10
                                associados
                            </p>
                        </div>
                                ...
                                favorites.length === 0 && (
                                    <div className="flex flex-col items-center gap-2 px-5 py-10 text-center">
                                        ...
                                        <p className="text-xs text-slate-400">
                                            Adicione códigos favoritos nas
                                            páginas de detalhe.
                                        </p>
                                    </div>
                                )}
                                    ...
                                    !activeSubData.error && (
                                        <div className="grid gap-4 lg:grid-cols-2">
                                            ...
                                            <div>
                                                ...
                                                {activeSubData.pcs.length >
                                                0 ? (
                                                    <ul className="max-h-64 space-y-0.5 overflow-auto">
                                                        {activeSubData.pcs.map(
                                                            (code) => (
                                                                <li
                                                                    key={`pcs-${code.id}`}
                                                                >
                ...
                <footer className="border-t border-slate-200 bg-white py-6 dark:border-slate-800 dark:bg-slate-900">
                    <div className="mx-auto max-w-6xl px-4 sm:px-6">
                        <p className="text-center text-xs text-slate-400">
                            © {new Date().getFullYear()} Medic@dex —
                            Classificação ICD-10-CM/PCS
                        </p>
                    </div>
                </footer>
            </div>
